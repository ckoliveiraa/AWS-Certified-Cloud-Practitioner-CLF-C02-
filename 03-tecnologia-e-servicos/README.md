# Módulo 3 — Tecnologia e Serviços de Nuvem

> **Domínio 3 do exame CLF-C02** — Peso: **34%** (o maior do exame!)

Este módulo cobre a infraestrutura global da AWS e os principais serviços de computação, armazenamento, banco de dados, rede, monitoramento, desenvolvimento e IA/ML.

---

## Objetivos de Aprendizagem

- Entender a **infraestrutura global** da AWS (Regiões, AZs, Edge).
- Identificar serviços de **computação, armazenamento, bancos de dados e rede**.
- Conhecer serviços de **monitoramento, automação e desenvolvimento**.
- Conhecer serviços de **analytics, IA/ML e integração**.

---

## Estrutura do Módulo

| # | Aula | Duração |
|---|------|---------|
| 3.1 | [Infraestrutura Global AWS](./3.1-infraestrutura-global.md) | 20 min |
| 3.2 | [Computação: EC2, Lambda, ECS/EKS, Beanstalk](./3.2-computacao.md) | 35 min |
| 3.3 | [Armazenamento: S3, EBS, EFS, FSx, Snow](./3.3-armazenamento.md) | 30 min |
| 3.4 | [Bancos de Dados: RDS, Aurora, DynamoDB, Redshift](./3.4-bancos-de-dados.md) | 30 min |
| 3.5 | [Rede: VPC, Route 53, CloudFront, Direct Connect](./3.5-rede.md) | 35 min |
| 3.6 | [Monitoramento e Gestão: CloudWatch, CloudFormation, SSM](./3.6-monitoramento-gestao.md) | 25 min |
| 3.7 | [Integração e Mensageria: SNS, SQS, EventBridge](./3.7-integracao-mensageria.md) | 20 min |
| 3.8 | [Desenvolvedor: Code*, Cloud9, X-Ray](./3.8-ferramentas-desenvolvedor.md) | 20 min |
| 3.9 | [Analytics, IA/ML: Athena, Glue, SageMaker, Bedrock](./3.9-analytics-ia-ml.md) | 25 min |

---

## Documentação Oficial (pt-BR)

- [Infraestrutura global AWS](https://aws.amazon.com/pt/about-aws/global-infrastructure/)
- [Amazon EC2](https://docs.aws.amazon.com/pt_br/ec2/)
- [AWS Lambda](https://docs.aws.amazon.com/pt_br/lambda/latest/dg/welcome.html)
- [Amazon ECS](https://docs.aws.amazon.com/pt_br/AmazonECS/latest/developerguide/Welcome.html)
- [Amazon EKS](https://docs.aws.amazon.com/pt_br/eks/latest/userguide/what-is-eks.html)
- [AWS Elastic Beanstalk](https://docs.aws.amazon.com/pt_br/elasticbeanstalk/latest/dg/Welcome.html)
- [Amazon S3](https://docs.aws.amazon.com/pt_br/AmazonS3/latest/userguide/Welcome.html)
- [Amazon EBS](https://docs.aws.amazon.com/pt_br/ebs/latest/userguide/what-is-ebs.html)
- [Amazon EFS](https://docs.aws.amazon.com/pt_br/efs/latest/ug/whatisefs.html)
- [AWS Snow Family](https://docs.aws.amazon.com/pt_br/snowball/latest/developer-guide/whatissnowball.html)
- [Amazon RDS](https://docs.aws.amazon.com/pt_br/AmazonRDS/latest/UserGuide/Welcome.html)
- [Amazon Aurora](https://docs.aws.amazon.com/pt_br/AmazonRDS/latest/AuroraUserGuide/CHAP_AuroraOverview.html)
- [Amazon DynamoDB](https://docs.aws.amazon.com/pt_br/amazondynamodb/latest/developerguide/Introduction.html)
- [Amazon Redshift](https://docs.aws.amazon.com/pt_br/redshift/latest/mgmt/welcome.html)
- [Amazon VPC](https://docs.aws.amazon.com/pt_br/vpc/latest/userguide/what-is-amazon-vpc.html)
- [Amazon Route 53](https://docs.aws.amazon.com/pt_br/Route53/latest/DeveloperGuide/Welcome.html)
- [Amazon CloudFront](https://docs.aws.amazon.com/pt_br/AmazonCloudFront/latest/DeveloperGuide/Introduction.html)
- [AWS Direct Connect](https://docs.aws.amazon.com/pt_br/directconnect/latest/UserGuide/Welcome.html)
- [Amazon CloudWatch](https://docs.aws.amazon.com/pt_br/AmazonCloudWatch/latest/monitoring/WhatIsCloudWatch.html)
- [AWS CloudFormation](https://docs.aws.amazon.com/pt_br/AWSCloudFormation/latest/UserGuide/Welcome.html)
- [AWS Systems Manager](https://docs.aws.amazon.com/pt_br/systems-manager/latest/userguide/what-is-systems-manager.html)
- [Amazon SNS](https://docs.aws.amazon.com/pt_br/sns/latest/dg/welcome.html)
- [Amazon SQS](https://docs.aws.amazon.com/pt_br/AWSSimpleQueueService/latest/SQSDeveloperGuide/welcome.html)
- [Amazon EventBridge](https://docs.aws.amazon.com/pt_br/eventbridge/latest/userguide/eb-what-is.html)
- [Amazon SageMaker](https://docs.aws.amazon.com/pt_br/sagemaker/latest/dg/whatis.html)
- [Amazon Bedrock](https://docs.aws.amazon.com/pt_br/bedrock/latest/userguide/what-is-bedrock.html)

---

## Recursos Complementares

- [Laboratórios práticos](./labs.md)
- [Glossário](./glossario.md)
- [Flashcards](./flashcards.md)
- [Simulado — Domínio 3](./simulado.md)
- [Checklist de revisão](./checklist.md)

---

[← Módulo anterior](../02-seguranca-e-conformidade/README.md) | [← Índice](../README.md) | [Próximo módulo →](../04-cobranca-precos-e-suporte/README.md)
